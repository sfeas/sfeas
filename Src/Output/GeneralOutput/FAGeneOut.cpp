...
//Description	: this program is definition of GeneOut for FASolu
...
void FAGeneOut::exec(double fNow) {
	Timer timerSolver("Output");

	if (isGeneOut()) {
		pltfile << "ZONE T= \"" << iStep << "-" << fNow << "\",N=" << nNode << ",E=" << nElem << ",DATAPACKING=POINT,ZONETYPE=FEBRICK" << "\n";

		updateU(CDVA, fNow, 0);		// amplitude
		for (int i = 0; i < nGENEOUT; i++) {
